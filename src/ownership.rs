...
//fixme:  fn dangle() -> &String {
//     let s: String = String::from("hi!");
//     //  область видимости перменной - функция, а указатель возвращается
//     //  в вызывающий контекст, значит ссылка недействительна - CE
//     &s
// }
...
pub fn test() {
    ...
    //  Явное ограничение времени жизни ссылочной переменной с помощью области видимости
    {
        let mut s: String = String::from("Hello");
        {
            let r: &mut String = &mut s;
            s.push_str("hello");
            //  -----> время жизни изменяемой ссылочной переменной закончилось
        }
        //  ----> с этого момента мы можем создать другую (mut или нет) ссылку на переменную
        //  без ошибой компиляции
        let c_r: &String = &s;
    }
    //  ВРЕМЯ ЖИЗНИ ССЫЛОЧНОЙ ПЕРЕМЕННОЙ

    {
        ...
        let r2: &String = &s;
        println!("{} and {}", r1, r2);
        //  область вивдимости ссылочной переменной продолжается до места
        //  где она последний раз использпользуется
        //  область видимости r1 и r2 заканчивается после println!

        //  ----> с этого момента мы можем создать другую (mut или нет) ссылку на переменную
        //  без ошибой компиляции
        let c_r: &mut String = &mut s;
        println!("{}", c_r);
    }
